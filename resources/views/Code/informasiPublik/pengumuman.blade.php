...
<html lang="en">
  <head>
    ...
    <title>Pengumuman | Kelurahan Temas</title>
    ...
    <main class="container mt-5">
      ...
      <div class="col-lg-8">
        @foreach ($announcement as $post)
          <article class="mb-5 d-flex align-items-start">
            <div style="width: 180px" class="text-center">
              <img style="max-height: 150px; max-width:150px; overflow:hidden" src="{{ asset('storage/' . $post->image) }}" alt="" class="image-fluid me-3">
            </div>
          <div class="flex-grow-1">
            <h3>{{ $post->title }}</h3>
            <p><small>{{ $post->created_at->diffForHumans() }} • {{ $post->user->name }}</small></p>
            <p>{{ $post->excerpt }}</p>
            <a href="/pengumuman/{{ $post->slug }}" class="btn btn-primary">Selengkapnya</a>
          </div>
          </article>
          @endforeach
      </div>
